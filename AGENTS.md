...
npm run audit:db      # audit from DB data
...
npm run sync:positions      # reconcile ghost positions
...
- Writes to `market_snapshots` table (2-day retention via pg_cron)

**Brain** (`src/trading-loop.ts` → `TradingLoop.runOnce()`) — dynamic schedule via `next_check_minutes`:
- With positions: min 10 min floor
- Without positions: min 30 min floor (quiet market)
- LLM suggests value, algorithmic floors override if too low
...
10. **Swarm or Single LLM** — if BTC volumeRatio > 1.5, `SwarmAgent` runs Blackboard Multi-Agent Debate; otherwise single `LLMClient.analyze()`
...
- `prompts.ts` — `buildSystemPrompt()` with comprehensive strategy. `buildEnrichedPrompt()` assembles all data + interpreted narratives. HOLD decisions use 1-word slug reasoning (token optimization).
...
**Swarm Blackboard Debate** (`swarm-agent.ts` + `swarm-blackboard.ts` + `blackboard-prompts.ts`):
- `SwarmAgent.getConsensus()` — Blackboard-pattern debate with shared state
- 6 personas: `risk_manager` (RM), `bull_thesis` (BT), `bear_thesis` (BA), `market_structure` (MS), `devils_advocate` (DA), `narrative_expert` (NE)
- Round 1: RM, MS run in parallel (+ NE via Grok if available)
- **Reactive DA** (`da-activation.ts`): activates AFTER round 1 votes. Conditions: exactly 1 action vote, or CLOSE exists. Uses Grok with `web_search` + `x_search` for real search-backed arguments. DA never votes HOLD.
- Judge reads aggregated blackboard, decides continue/stop, produces final verdict
- Up to 4 rounds; fingerprint caching skips debate if market unchanged
...
- Used by: SwarmAgent (narrative_expert + DA), FlashCrashScanner, GrokGrounder
...
- `regime-classifier.ts` — `classifyRegime()`: 6 regimes (BullTrend, BearTrend, Range, Breakout, Capitulation, Scalping) with confidence scoring based on F&G, ADX, EMA, VWAP, volume
- **Capitulation** requires volume > 3x OR (F&G < 15 AND volume > 1.5x). F&G alone does NOT trigger Capitulation.
...
- `leverageMultiplier` respects `minLeverage` from config (floor, not 1)
- **No regime_override** — classifier is single source of truth. LLM controls actions/confidence/sizing, not system filters.
- `regime-hysteresis.ts` — prevents rapid regime flapping
...
- `news-market-fusion.ts` — correlates news signals with market snapshots for reaction detection
...
**Supabase PostgreSQL** — unified telemetry store. 20 tables covering the full chain: prompt → decision → execution → P&L. Enabled via `DATABASE_URL` env var.

- `connection.ts` — PG pool via `pg` npm, `initPool()` / `closePool()`
- `types.ts` — TypeScript interfaces matching all tables
...
**Retention:** `market_snapshots` and `indicator_snapshots` auto-delete rows > 2 days via pg_cron (04:00 UTC daily).

**Requires:** `DATABASE_URL` env var. Bot runs fine without it (graceful fallback, JSONL only).

## Dashboard (`dashboard/`)

Separate React + Vite app reading from Supabase REST API. CSS variable theme system with light/dark toggle.
...
- **Grok features** require `XAI_API_KEY` env var for FlashCrashScanner, SwarmAgent (narrative_expert + DA), GrokGrounder.
...
- **Binance API is IP-whitelisted** — can only call from GCP VM. Run scripts on VM via SSH, not locally.
- **`leverageMultiplier`** — applied after RiskManager. Uses `Math.max(minLeverage, ...)` not `Math.max(1, ...)`.
- **Regime is algorithmic only** — no `regime_override` from LLM. Classifier is single source of truth.
