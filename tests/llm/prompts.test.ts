...
describe('buildUserPrompt', () => {
  it('should inject filterWarning if provided', () => {
    const prompt = buildUserPrompt({
      snapshots: [],
      indicators: new Map(),
      portfolio: { balanceUsd: 100, availableUsd: 100, positions: [], sessionPnl: 0, drawdownPct: 0 },
      signals: [],
      news: [],
  ...
  it('should render positionContexts with SL/TP and entry thesis', () => {
    const prompt = buildUserPrompt({
      ...
      indicators: new Map(),
      portfolio: { balanceUsd: 100, availableUsd: 80, positions: [{ pair: 'BTCUSDT', side: 'LONG', entryPrice: 70000, heldHours: 2, unrealizedPnlPct: 2.8, leverage: 10 }], sessionPnl: 5, drawdownPct: 0 },
      signals: [],
      news: [],
      fearGreed: { value: 50, label: 'Neutral' },
  ...
  it('includes recent decisions in prompt', () => {
    const data: EnrichedPromptData = {
      snapshots: [],
      indicators: new Map(),
      portfolio: { balanceUsd: 178, availableUsd: 178, sessionPnl: -6, drawdownPct: 0, positions: [] },
      signals: [], news: [],
  ...
  it('handles string sl_price/tp_price from DB without crashing', () => {
    const data: EnrichedPromptData = {
      ...
      indicators: new Map(),
      portfolio: {
        balanceUsd: 178, availableUsd: 150, sessionPnl: -6, drawdownPct: 0,
...
describe('buildCritiquePrompt', () => {
  it('builds critique prompt with other experts, filters self', () => {
